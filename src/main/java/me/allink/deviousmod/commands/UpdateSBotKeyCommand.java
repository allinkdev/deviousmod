...
public class UpdateSBotKeyCommand extends ChatCommandBase {
    Utilities utilities;
    ChatUtil chatUtil;

    public UpdateSBotKeyCommand(int minimumArguments, String command, String usage, String description, List<String> aliases) {
    ...
    public void execute(String[] args) throws NotEnoughArgumentsException {
        ...
        DeviousModClient.CONFIG.sbotKey = key;
        DeviousModClient.getInstance().saveConfig(DeviousModClient.CONFIG);
        chatUtil.sendMessage(new LiteralText(String.format("Updated your SBot key to: %s", key)).formatted(Formatting.GREEN));
    }
