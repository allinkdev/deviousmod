...
public class SValCommand extends ChatCommandBase {
    Utilities utilities;
    ChatUtil chatUtil;

    public SValCommand(int minimumArguments, String command, String usage, String description, List<String> aliases) {
    ...
    public void execute(String[] args) throws NotEnoughArgumentsException {
        ...
        try {
            ...
            String time = "" + (System.currentTimeMillis() / 5000);
            String key = DeviousModClient.CONFIG.sbotKey;
            String username = client.player.getName().getString();
            String raw = message.replaceAll("&[0-9a-fklmnor]", "") + ";" + username.replaceAll("§[0-9a-fklmnor]", "") + ";" + time + ";" + key;
            ...
            chatUtil.sendMessage(new LiteralText("Oops! Did you forget to set your SBot key? Check the logs for more details...").formatted(Formatting.RED));
            e.printStackTrace();
        }
    }
}
